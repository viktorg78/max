...
$typTeh2=strip_tags($_POST['typTeh2']); // тип пехники
...
$arend=0; $zalog=0; $yslygi=0;

$k_arend = strip_tags($_POST['k_arend']);// краткосрочная аренда
if ($k_arend) $arend=$arend+1; 
$d_arend = strip_tags($_POST['d_arend']);// долгосрочная аренда 
if ($d_arend) $arend=$arend+2;

$da_zalog = strip_tags($_POST['da_zalog']);// да залог
if($da_zalog) $zalog=$zalog+1; 
$net_zalog = strip_tags($_POST['net_zalog']);// нет залог
if($net_zalog) $zalog=$zalog+2;

$da_yslygi = strip_tags($_POST['da_yslygi']);// да услуги водителя
if ($da_yslygi) $yslygi = $yslygi+1;
$net_yslygi = strip_tags($_POST['net_yslygi']);// нет услуги водителя
...
//	$imj2=addslashes(file_get_contents($_FILES['myfile'][1]['tmp_name']));
	...
	setMes_arend($typTeh, $typTeh2, $proizvod, $model, $opis, $cesh, $otvet, $telk, $telk_skr, $oplata, $dosavka, $imj1, $imj2, $imj3, $imj4, $imj5, $imj6, $imj7, $imj8, $imj9, $id_logo, $kontrogent, $name1, $name2, $name3, $name4, $site, $arend, $zalog, $yslygi);//запись юзера в БД
