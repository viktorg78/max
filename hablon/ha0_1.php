...
//	$id_logo = $id_logo[0]["id"];
    $masseg0 = getBDMessage($id_logo, 1); // чтение из таблицы праметры
  //  closeDB();// закрыть БД
  echo '<table class="ob1">';
    echo '<tr><td>ОБЪЯВЛЕНИЯ О ОРЕНДЕ</td></tr>';
    echo '</table>';
    for ($i=0; $i<count($masseg0);$i++){ 
        ...
        <table class="ob1">
            ...
            <tr><td colspan="2"><button class="dell" data-id="'.$masseg0[$i]['id'].'" data-key="'. $key.'" data-key2="'.$key2.'" data-p="0">Удалить объявление</button></td></tr>
        </table>';
    }
?>
